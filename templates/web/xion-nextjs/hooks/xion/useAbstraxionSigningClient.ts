...
export const useAbstraxionSigningClient = (): {
  ...
  const { isConnected, abstraxionAccount, granterAddress, rpcUrl, logout } = useContext(AbstraxionContext);

  const [signArbWallet, setSignArbWallet] = useState<SignArbSecp256k1HdWallet | undefined>(undefined);
  ...
  const [abstractClient, setAbstractClient] = useState<GranteeSignerClient | undefined>(undefined);
